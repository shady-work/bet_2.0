        ...
        methods:
            {
                ...
                //获取egg数据
                get_egg_history(date)
                {
                    let url = ``;
                    if(date)
                    {
                        url = `${this.global.config.API}egg/history/lottery?per_page=300&range=${date}`;
                    }
                    else
                    {
                        url = `${this.global.config.API}egg/history/lottery?per_page=300`;
                    }
                    this.isShow = false;
                    this.$http.get(url).then(function(res)
                    {
                ...
                get_data_by_date()
                {
                    this.get_egg_history(this.value1);
                },
