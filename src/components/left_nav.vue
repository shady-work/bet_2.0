    ...
    methods:
      {
        ...
        //获取最新一期的开奖号码，和结算用户的上期中奖金额
        getLastExpectSsc()
        {
          let that = this;
          this.ssc.timeId3 = setInterval(function()
          {
            that.$http.get(that.global.config.API + 'ssc/lastLty', {}).then(function (res)
            {
              //获取到最新一期的数据
              let data = res.data;
              if(res.status == 200)//请求成功
              {
                that.$store.state.cqssc.open_codes  = data.opencode;
                //判断是否为最新有一期数据
                if(parseInt(data.expect) == parseInt(that.$store.state.cqssc.lastExpect)+1)
                {
                  that.$store.state.cqssc.lastExpect  = data.expect;
        ...
        {
          let that = this;
          this.pk10.timeId3 = setInterval(function()
          {
            that.$http.get(that.global.config.API + 'pk10/lastLty', {}).then(function (res)
            {
              //获取到最新一期的数据
              let data = res.data;
              if(res.status == 200)//请求成功
              {
                that.$store.state.pk10.open_codes  = data.opencode;
                //判断是否为最新有一期数据
                if(parseInt(data.expect) == parseInt(that.$store.state.pk10.lastExpect)+1)
                {
                  that.$store.state.pk10.lastExpect  = data.expect;
                  that.$store.state.pk10.bingo_money = data.lucky_money;
                  if(that.$store.state.pk10.bingo_money > 0 )
                  {
                    that.$notify({
                      title: `北京赛车PK拾`,
                      message: `第<b style="color:blue"> ${data.expect}</b>期中奖 ￥<b style="color:red">${data.lucky_money}</b>。`,
        ...
        {
          let that = this;
          this.cake.timeId3 = setInterval(function()
          {
            that.$http.get(that.global.config.API + 'cake/lastLty', {}).then(function (res)
            {
              //获取到最新一期的数据
              let data = res.data;
              if(res.status == 200)//请求成功
              {
                that.$store.state.cake.open_codes  = data.details.ball_0;
                //判断是否为最新有一期数据
                if(parseInt(data.expect) == parseInt(that.$store.state.cake.lastExpect)+1)
                {
                  that.$store.state.cake.lastExpect  = data.expect;
                  that.$store.state.cake.bingo_money = data.lucky_money;
                  if(that.$store.state.cake.bingo_money > 0 )
                  {
                    that.$notify({
                      title: `加拿大28`,
                      message: `第<b style="color:blue"> ${data.expect}</b>期中奖   ￥<b style="color:red">${data.lucky_money}</b>。`,
        ...
        {
          let that = this;
          this.egg.timeId3 = setInterval(function()
          {
            that.$http.get(that.global.config.API + 'egg/lastLty', {}).then(function (res)
            {
              //获取到最新一期的数据
              let data = res.data;
              if(res.status == 200)//请求成功
              {
                that.$store.state.egg.open_codes  = data.details.ball_0;
                //判断是否为最新有一期数据
                if(parseInt(data.expect) == parseInt(that.$store.state.egg.lastExpect)+1)
                {
                  that.$store.state.egg.lastExpect  = data.expect;
                  that.$store.state.egg.bingo_money = data.lucky_money;
        ...
        get_all_last_open_codes()
        {
          ...
          this.$http.get(this.global.config.API + 'ssc/lastLty', {}).then(function (res)
          {
            ...
            if(res.status == 200)//请求成功
            {
              this.$store.state.cqssc.open_codes  = data.opencode;
              this.$store.state.cqssc.lastExpect  = data.expect;
              this.$store.state.cqssc.bingo_money = data.lucky_money;
            }
          }).then(function()
          {
            this.getLastExpectSsc();
          });
          /****************************************pk10*************************************************************************/
          this.$http.get(this.global.config.API + 'pk10/lastLty', {}).then(function (res)
          {
            ...
            if(res.status == 200)//请求成功
            {
              this.$store.state.pk10.open_codes  = data.opencode;
              this.$store.state.pk10.lastExpect  = data.expect;
              this.$store.state.pk10.bingo_money = data.lucky_money;
            }
          }).then(function()
          {
            this.getLastExpectPk10();
          });

          /****************************************cake*************************************************************************/
          this.$http.get(this.global.config.API + 'cake/lastLty', {}).then(function (res)
          {
            //获取到最新一期的数据
            let data = res.data;
            if(res.status == 200)//请求成功
            {
              this.$store.state.cake.open_codes  = data.details.ball_0;
              this.$store.state.cake.lastExpect  = data.expect;
              this.$store.state.cake.bingo_money = data.lucky_money;
            }
          }).then(function()
          {
            this.getLastExpectCake();
          });


          /****************************************egg*************************************************************************/
          this.$http.get(this.global.config.API + 'egg/lastLty', {}).then(function (res)
          {
            //获取到最新一期的数据
            let data = res.data;
            if(res.status == 200)//请求成功
            {
              this.$store.state.egg.open_codes  = data.details.ball_0
              this.$store.state.egg.lastExpect  = data.expect;
              this.$store.state.egg.bingo_money = data.lucky_money;
            }
          }).then(function()
          {
            this.getLastExpectEgg();
          });
        }
        /****************************************我是华丽的分割线*************************************************************************/
      },
...
<style scoped>
  ...
  .active {
    ...
    background: -webkit-linear-gradient(left, #ff6331, rgba(200, 200, 200, 0.1));
    background: -o-linear-gradient(right, #ff6331, rgba(200, 200, 200, 0.1));
    background: -moz-linear-gradient(right, #ff6331, rgba(200, 200, 200, 0.1));
    background: linear-gradient(to right, #ff6331, rgba(200, 200, 200, 0.1));
